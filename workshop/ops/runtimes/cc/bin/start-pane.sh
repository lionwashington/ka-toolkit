#!/bin/bash
# cc/bin/start-pane.sh — Claude Code process entrypoint inside a tmux pane.
#
...
#
# Usage: cc/start-pane.sh <pane-name> <expected-cwd> [extra claude args...]
...
if [ "$_has_resume" -eq 0 ]; then set -- "$@" --resume latest; fi
resolve_resume_args "$@"
