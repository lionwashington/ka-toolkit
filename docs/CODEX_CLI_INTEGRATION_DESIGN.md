...
- App Server 仅通过 stdio 或本机 socket 访问。
...
状态（2026-07-19）：核心链路已通过，包括真实 App Server 的持久化 thread、跨 App Server 进程 resume 和上下文连续性。并发、interrupt、畸形 event 与 supervisor 有界重启仍需补齐后，才进入 Phase 2。
...
- [pwrdrvr/openclaw-codex-app-server](https://github.com/pwrdrvr/openclaw-codex-app-server)
