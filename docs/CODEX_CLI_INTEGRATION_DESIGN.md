...
Workshop pane
  ...
  |- codex --remote <endpoint> resume <canonical-thread-id>  (TUI)
...
live endpoint plus the TUI's canonical thread ID with Channel's loopback control API. The
default thread is the most recent thread whose recorded cwd exactly matches the
mate cwd; `args: [resume, <thread-id>]` overrides it and must pass the same cwd
check. Channel resumes that registered thread and never creates a second
Telegram-only thread. The registration loop heals
...
Status (2026-07-20): complete. Telegram live text/image turns passed; a real
...
Status (2026-07-20): complete. The Codex adapter implements TUI launch with a
