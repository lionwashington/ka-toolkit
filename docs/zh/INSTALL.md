...
| core CLI（供 `/kb` 使用） | `kb/core/dist/` | 直接拷贝（tsup 自包含 ESM，并生成 runtime `package.json`） |
...
`--switch` 会把 `~/.claude.json` 的 MCP 条目重新指向 `$KA_HOME/kb/mcp/*`，把 `ka` 符号链接指向 `$KA_HOME/shared/bin/ka`，把 cron plists 和 CC hook 路径重新指向 runtime，把任何遗留的守护进程 `state.json` 迁移进 `$KA_HOME/channels/<kind>-daemon/` 并重启活动守护进程，并将技能链接到 Claude Code 与 Codex 的发现目录。（守护进程**密钥**不会自动迁移 —— 请先填好 `config/secrets.yaml channels.<kind>`。）每一步都会先留一份 `.pre-switch-*` 备份。**没有 `--rollback` 命令** —— 如果发现哪里不对，手动还原：把各个 `~/.claude.json.pre-switch-*` / `~/.claude/settings.json.pre-switch-*` / `*.plist.pre-switch` 拷回原处，把 `~/.local/bin/ka` 符号链接指回它的 `.pre-switch-target`，再重启旧 daemon。（确认无误前不要跑 `--cleanup-old` —— 它会删掉这些备份。）
