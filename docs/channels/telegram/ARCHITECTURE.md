...
5. **attachment download and batching**: an update is more than just `message.text`. `extractAttachment` recognizes `photo` (takes the largest size) / `document` / `video` / `audio` / `voice` / `animation` / `video_note` / `sticker` → `bot.api.getFile` + download to `attachments/<update_id>-<file>`. A standalone attachment retains the legacy `attachment_path` / `attachment_kind` metadata. Telegram album members sharing the same `chat_id + media_group_id` are durably buffered, ordered by `update_id`, downloaded concurrently, and delivered once with `attachment_count` plus ordered `attachment_N_path` / `attachment_N_kind` fields. The first item is also mirrored to the legacy singular fields. Codex receives the ordered images as multiple `localImage` inputs in one turn; MCP consumers receive one notification and inspect only its named paths.
...
6. **dispatch → fanout**: `dispatch(targetName, content, meta)` → iterates **all** sessions in `byName[target]`, with `fanout` using `Promise.allSettled` to deliver in parallel + a 5s timeout per delivery (a half-dead session does not block live consumers, avoiding head-of-line blocking). method `notifications/claude/channel`, params `{ content, meta }`.
   - 🔴 **every meta field must be `String()`** (`fanout` uniformly does `Object.entries(...).map([k,v]=>[k,String(v)])`). Numeric/object fields cause the dev-channels consumer to **silently drop the entire notification** (the top invariant). The telegram inbound meta includes `{ chat_id, sender_name, sender_id, message_id, ts, [attachment_path], [attachment_count], [attachment_N_path] }`, all String.
...
**Requirement**: the user wants multiple peer agent processes to coordinate **without** using Claude Code's team-mate mechanism (which implicitly shares a workspace/cwd and has one process spawn the others). Workshop mates instead run in independent workspaces, start and stop independently, and collaborate purely via messages. telegram-channel already connects "external user ↔ multiple agents"; cc2cc uses the same mechanism for agent-to-agent communication.
