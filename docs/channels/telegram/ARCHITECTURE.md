...
4. **routing-prefix parse** `parseRoutingPrefix(text || caption)`:
   - Accepts the prefix `to` (case-insensitive) or `2` (homophone); the target may be a channel name or a **number**; the colon `:`/`：` is optional.
   - No prefix → `main`, full text.
   - prefix + colon → explicit targeting (routes even if the target is offline, to give feedback).
   - prefix without colon → only targets the channel if it is **online**, otherwise → `main`.
