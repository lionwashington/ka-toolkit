...
hits="$(
    find "$REPO/shared" "$REPO/workshop" "$REPO/cron" "$REPO/kb/ops" "$REPO/channels/ops" -name '*.sh' \
...
out="$(BASH_COMPAT=3.2 DRY_RUN=1 OPS_CONFIG=/tmp/cfg.yaml \
    bash "$REPO/shared/bin/ka" workshop --dry-run 2>&1)"
...
BASH_COMPAT=3.2 bash -c "source '$REPO/workshop/ops/tmux-helpers.sh'; echo sourced" \
