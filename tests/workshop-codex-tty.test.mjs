...
test('launcher establishes keyboard mode before TUI and restores shell state on failure', () => {
  const source = readFileSync(new URL('../workshop/ops/runtimes/codex/bin/start-pane.sh', import.meta.url), 'utf8')
  const fn = source.match(/run_codex\(\) \{[\s\S]*?\n\}/)[0]
  const shell = `${fn}\nCODEX_MCP_OVERRIDES=(); APP_SERVER_ENDPOINT=test
codex() { stty -a; return 23; }
before=$(stty -g)
run_codex
result=$?
[ "$result" = 23 ] && [ "$(stty -g)" = "$before" ] || exit 1
echo RESTORED
`
  const python = `import os,pty,subprocess,sys,select
pid,fd=pty.fork()
if pid==0:
...
assert b'-icanon' in data and b'-echo ' in data and b'-icrnl' in data,data
assert b'RESTORED' in data,data
`
  const result = spawnSync('python3', ['-c', python, shell], { encoding: 'utf8', timeout: 10000 })
