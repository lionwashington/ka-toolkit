#!/usr/bin/env node
// Opt-in real-binary regression. No production configuration/authentication is
// read, no production tmux socket is used, and model traffic stays on loopback.
// Usage: node tests/manual/codex-remote-startup.mjs
...
const trustHelper = fileURLToPath(new URL('../../workshop/ops/runtimes/codex/hook-trust-overrides.mjs', import.meta.url))
const detachedHelper = fileURLToPath(new URL('../../workshop/ops/runtimes/codex/detached-process.mjs', import.meta.url))
...
async function runScenario(name, useOverlay) {
  const originalConfig = readFileSync(join(root, 'config.toml'), 'utf8')
  const args = []
  if (useOverlay) {
    const probe = spawnSync(process.execPath, [trustHelper, cwd, '--'], { env, encoding: 'utf8', timeout: 25_000 })
    assert.equal(probe.status, 0, 'isolated trust helper failed')
    assert.match(probe.stdout, /^hooks\.state=\{[^\n]+\}\n$/u)
    args.push('-c', probe.stdout.trim())
  }
  const listener = createTcpServer()
  await new Promise(done => listener.listen(0, '127.0.0.1', done))
  const endpoint = `ws://127.0.0.1:${listener.address().port}`
  await new Promise(done => listener.close(done))
  const sidecar = spawn(process.execPath, [detachedHelper, 'codex', ...args, '--dangerously-bypass-hook-trust', 'app-server', '--listen', endpoint], {
...
try {
  await runScenario('flag-only', false)
  await runScenario('overlay', true)
  writeFileSync(join(root, 'config.toml'), config.replace('command="true"', 'command=":"'))
  await runScenario('changed-hook-overlay', true)
  console.log(JSON.stringify({ passed: true, loopbackModelUsed: modelRequests >= 3 }))
} finally {
