...
[ -f "${KA_REPO_ROOT:-}/packages/core/dist/config-cli.js" ] && cfgcli="$KA_REPO_ROOT/packages/core/dist/config-cli.js"
...
if [ -f "$SETTINGS" ] && command -v python3 >/dev/null 2>&1; then
    ...
    hook_rows="$(python3 - "$SETTINGS" "$runtime_hooks_dir" "$KA_REPO_ROOT" <<'PY' 2>/dev/null
import json, sys
