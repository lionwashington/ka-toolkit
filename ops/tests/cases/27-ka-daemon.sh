...
echo "[1/5] ka daemon help lists the verbs"
out="$(NO_COLOR=1 HOME="$TMP/h1" "$KA" daemon help 2>&1)"
for v in start stop restart status config; do
    echo "$out" | grep -q "ka daemon $v" || { echo "FAIL: help missing verb '$v'"; echo "$out"; exit 1; }
done
echo "    ok"

echo "[2/5] unknown verb → exit 2"
set +e; NO_COLOR=1 HOME="$TMP/h2" "$KA" daemon frobnicate >/dev/null 2>&1; rc=$?; set -e
[ "$rc" -eq 2 ] || { echo "FAIL: unknown verb rc=$rc (want 2)"; exit 1; }
echo "    ok"

echo "[3/5] no config → telegram; status reports down on 9877 + exit 1"
h="$TMP/h3"; mkdir -p "$h"
set +e; out="$(NO_COLOR=1 HOME="$h" "$KA" daemon status 2>&1)"; rc=$?; set -e
...
set +e; out="$(NO_COLOR=1 HOME="$h" KA_CONFIG="$h/config.yaml" "$KA" daemon status 2>&1)"; set -e
...
h="$TMP/h5"; mkdir -p "$h"
set +e; out="$(NO_COLOR=1 HOME="$h" "$KA" daemon start 2>&1)"; rc=$?; set -e
